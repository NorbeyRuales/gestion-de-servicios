-- Extrae los UUID como texto JSON sin conservar las comillas del valor.
create or replace function public.create_invoice_from_orders_v2(payload jsonb)
returns jsonb
language plpgsql
security definer
set search_path = ''
...
begin
  ...
  for order_id in
    select value::uuid
    from jsonb_array_elements_text(selected_orders) as selected(value)
  loop
...
-- Mantiene compatibilidad con clientes que todavía llamen el nombre anterior.
...
set search_path = ''
as $$
  select public.create_invoice_from_orders_v2(payload);
$$;

revoke all on function public.create_invoice_from_orders_v2(jsonb) from public;
revoke all on function public.create_invoice_from_orders(jsonb) from public;
grant execute on function public.create_invoice_from_orders_v2(jsonb) to authenticated;
