...
function AssetForm({ areas, categories, initial, saving, onCancel, onSave }: { areas: AreaRecord[]; categories: string[]; initial?: AssetRecord; saving: boolean; onCancel: () => void; onSave: (value: AssetValue) => Promise<void> }) {
  ...
  const [customCategory, setCustomCategory] = useState(Boolean(initial?.category && !categories.includes(initial.category)));
  const [error, setError] = useState("");
...
export function AssetManagement({ branchId, canAdminister = false, onNewWorkOrder, onOpenOrder }: { branchId: string; canAdminister?: boolean; onNewWorkOrder: (assetId: string) => void; onOpenOrder: (orderId: string) => void }) {
  ...
  const [categories, setCategories] = useState<string[]>([...restaurantEquipmentCategories]);
  ...
  const load = useCallback(async () => {
    ...
    const [areasResult, assetsResult, categoriesResult] = await Promise.all([
      ...
      supabase.from("asset_categories").select("name").eq("is_active", true).order("sort_order").order("name"),
    ]);
    ...
    if (!categoriesResult.error && categoriesResult.data?.length) setCategories(categoriesResult.data.map((category) => category.name));
    setLoading(false);
  }, [branchId]);
  ...
  return <section className="mt-6">
    ...
    {editingAsset && <Modal title={editingAsset === "new" ? "Nuevo equipo" : "Editar equipo"} onClose={() => setEditingAsset(null)}><AssetForm areas={areas} categories={categories} initial={editingAsset === "new" ? undefined : editingAsset} saving={saving} onCancel={() => setEditingAsset(null)} onSave={saveAsset} /></Modal>}
