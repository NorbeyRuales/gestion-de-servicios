...
export function InvoiceEditForm({ invoice, branches, onCancel, onSaved }: InvoiceEditFormProps) {
  ...
  return <form onSubmit={(event) => void save(event)} className="mt-4 rounded-xl border border-blue-200 bg-blue-50/40 p-4">
    ...
    <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
      ...
      <label className="text-xs font-semibold">Descuento<input type="number" min="0" max={subtotal} inputMode="numeric" placeholder="$ 0" className={inputClass} value={discount || ""} onFocus={(event) => event.currentTarget.select()} onChange={(event) => setDiscount(Number(event.target.value))} /></label>
