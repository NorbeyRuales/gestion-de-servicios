  ...
  return <section className="overflow-hidden rounded-xl border border-border bg-card shadow-sm">
    ...
    <div className="space-y-3 p-4">
      {items.map((item) => <article key={item.id} className="grid items-end gap-2 rounded-xl border border-border p-3 sm:grid-cols-2 lg:grid-cols-[150px_1fr_100px_150px_120px_40px]">
        ...
        <label className="text-xs font-semibold">Cantidad<input type="number" min="0.01" step="0.01" inputMode="decimal" placeholder="0" className={`${inputClass} mt-1`} value={item.quantity || ""} onFocus={(event) => event.currentTarget.select()} onChange={(event) => update(item.id, { quantity: Number(event.target.value) })} /></label>
        <label className="text-xs font-semibold">Valor unitario<input type="number" min="0" step="1" inputMode="numeric" placeholder="$ 0" className={`${inputClass} mt-1`} value={item.unit_price || ""} onFocus={(event) => event.currentTarget.select()} onChange={(event) => update(item.id, { unit_price: Number(event.target.value) })} /></label>
        <div className="pb-2 text-right text-xs"><span className="block text-muted-foreground">Subtotal</span><strong className="font-semibold tabular-nums">{money(Number(item.quantity || 0) * Number(item.unit_price || 0))}</strong></div>
